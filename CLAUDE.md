...
- **Astra bundle staleness rides an opaque per-host mtime token.** `/astra-bundle` embeds `mtime: <token>` in its JSON response (server `statSync(filePath).mtimeMs` for local, `stat -c %Y` over SSH for remote). `/astra-mtime/{originId}{absPath}` returns the same token without running buildBundle — the contract is "compares equal iff unchanged for this path," so format differs across local/remote but a given path always uses the same scheme. Vellum's `AstraFilePanel` listens for `window.focus`; if the probe returns a different token than the one stored on `bundleResult.mtime`, an internal cache-bust counter bumps and bundle/iframe/source effects all retrigger through the same path the host's manual Refresh uses. Annotations skip the bump (they re-anchor by `selectedText + surrounding context`). Token-comparison errors are swallowed — better stale than spinning. `vellum-reader/astra-bundle-staleness-on-focus`
- **Astra paper-view rung falls through to linear under ~280 px host width.** The lightcone paper-view template lays out for a ~720 px column; below ~280 px it crops the title alone (e.g. only "DESI 2024 III:" shows) and the body is past the right edge — useless inside a sticky-note pin at deep zoom-out. `<AstraFilePanel>` ResizeObserves `.astra-paper-view-host` and substitutes `linear` when paper-view is the picked rung and host width drops below `PAPER_VIEW_MIN_WIDTH = 280`. The picker still shows the user's actual pick (so paper-view "stays selected" across the zoom range); a small gold-rule notice — "paper-view needs more room — showing linear at this size." — explains the divergence. Same fallthrough pattern as the bundle-unavailable case, just inverse direction. Modal mounts always sit above the threshold so the substitute never fires there. `vellum-reader/vellum-native-astra-renderer`
- **`AstraPicker` rungs clip past `.astra-paper-view-host`'s `overflow-x: hidden` at narrow card widths.** The host scrolls vertically (`overflow: hidden auto`); any inline-flex picker that doesn't wrap will overflow the card's right edge at the camera's deep zoom-out floor (~192 px) and the rightmost rung — `personal` — gets clipped both visually and for hit-testing. Symptom: `document.elementsFromPoint(personalCenter)` returns `[CANVAS, BODY, HTML]` rather than the button, even though the bounding-rect API still reports a layout box. Fix is `flex-wrap: wrap` on `.astra-picker` and `.astra-picker__options` in `vellum/src/vellum.css` so rungs that don't fit drop to row 2; modal and deep-zoom-in cards stay single-row. **A11y-tree-via-`@ref` clicks miss this kind of viewport-clip bug** because the ref targets the button regardless of pixel paint state — pixel hit-testing (`elementFromPoint` or a CDP mouse click at coordinates) is what catches reachability inside the structural a11y contract. `vellum-reader/astra-picker-overflow-deep-zoom`
...
- **Astra rung typography sized in `em` doesn't scale with the host.** The astra renderer mounts in two surfaces with very different widths: the workspace modal (always > 1000 px on desktop) and pin cards (200-1000 px depending on camera zoom). Type rules sized in absolute units (`em` against an 18px body, `px`, `rem`) render the same in both. The personal rung's `2.6em` title (~46.8 px) was designed for ~720 px columns; at the camera's deep zoom-out floor (~200 px host) the same title wraps to ~10 lines and dominates the entire visible card. Fix is **container queries**: `container-type: inline-size; container-name: astra-host;` on `.astra-paper-view-host`, then `clamp(min, calc(... + Ncqi), max)` on the type properties so they scale with the host's inline size. Use `@container astra-host (max-width: 479px)` only for binary affordances that don't have a sensible interpolated middle (drop-cap on/off, padding 1em vs 0.6em). **Don't use media queries** for this — they key off the viewport, which is the same in both surfaces. Container queries are widely supported on vellum's modern build target (Chrome 105+, Safari 16+, Firefox 110+), no `@supports` fallback needed. Same root cause class as the picker overflow — control sized in absolute units inside a host that shrinks past the design floor. `vellum-reader/astra-personal-rung-typography-clamp`
- **Astra picker collides with pin chrome at narrow widths.** Same diagnostic family as the typography clamp, applied to `.astra-picker`. The picker font (`0.92em`) and padding (`0.5em / 1em`) didn't scale with the host, so at the camera's 192 px deep-zoom-out floor the wrapped picker took ~125 px (47% of the card height). Fix 1: cqi-clamp font (`clamp(11px, calc(0.5em + 1.5cqi), 0.92em)`) and padding (`clamp(2px, 1.5cqi, 0.5em)` / `clamp(0.4em, 2.5cqi, 1em)`) on `.astra-picker` and `.astra-picker__options` `gap` — picker drops to ~50 px at deep zoom, modal/zoomed-in cards untouched. Fix 2: with options now packing on one line, they extended into the pin's top-right chrome cluster (`.dom-pin-affordances` from `renderAffordances` in `src/render/DomPinLayer.ts`, `position: absolute; top: 4px; right: 4px; z-index: 4; pointer-events: auto`, ~38 px wide); pixel hit-tests on the rightmost `personal` button routed to `.dom-pin-affordance-menu` / `.dom-pin-affordance-close` instead of the picker. Reservation: `@container astra-host (max-width: 320px) { .astra-picker { padding-right: 44px; } }`. Modal mounts always sit above 1000 px host so the rule is inert there. **A11y-`@ref` clicks miss this** for the same reason `astra-picker-overflow-deep-zoom` did — the ref targets the button regardless of paint; `document.elementsFromPoint(buttonCenter)` is the test that catches it. `vellum-reader/astra-picker-clamp-and-chrome-clear`
- **`DomPinLayer` injects `.dom-pin-vellum-shell h1/h2/h3 { font-size: <fixed em> }` (0,1,1) which shadows single-class astra typography rules (0,1,0).** Stylesheet at `src/render/DomPinLayer.ts:1246-1249` gives every card a baseline reading hierarchy (`h1 1.25em, h2 1.15em, h3 1.05em`). Class+element selector beats single-class astra rules on specificity — a `font-size: clamp(...)` on `.astra-paper-view__title` looks fine in the modal but is silently overridden by the dom-pin rule once mounted in a card. The personal rung's `.astra-paper-view--personal .astra-paper-view__title` (0,2,0) survived because its specificity already cleared the bar. **Rule:** astra typography rules that live inside `.dom-pin-vellum-shell` need at least (0,2,0). Cheapest authoring is `.astra-paper-view .astra-paper-view__X { ... }` (repeated parent class). Same trap class as the picker overflow / pin-chrome collision — render-time is fine, the bug is in card-time cascade. `vellum-reader/astra-linear-rung-typography-clamp`
