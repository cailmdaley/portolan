...
;(function () {
  ...
  function getClaimContext(el) {
    // Walk up DOM for explicit data attributes
    let node = el
    ...
    // Fallback: check if inside claim panel and read globals
  ...
  function autoTagClaimPanel() {
    ...
    // Tag artifact images when data-artifact is missing
    ...
    // Request annotation loading on claim change
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.altKey) return // Alt+click for lightbox
    ...
    // Remove pin on cancel; keep on save
  ...
  function showPinPopover(marker, ann, claimId) {
    ...
    requestAnimationFrame(() => {
      ...
      if (r.right > window.innerWidth - margin) {
        popover.style.left = Math.max(margin, rect.left - r.width - 8) + 'px'
      }
      if (r.bottom > window.innerHeight - margin) {
        popover.style.top = Math.max(margin, window.innerHeight - r.height - margin) + 'px'
      }
    })
