...
- **Keep enriching the analysis.** 21 nodes formalized. Spine complete. **20/21 nodes resolved** (updated 2026-04-07): added insights to `350-mock-glass-covariance-cl` (covariance matrix structure: 64×64 EE+BB, Hartlap ≈ 0.82) and `bb-covariance-blind` (BB covariances blind-independent: COSEBIS exactly so, pure E/B ~13%, harmonic ~8%). Only `root` node remains "open" (by design — it's the container). Remaining periphery (6 nodes: blinding-framing-for-talk, bump-no-longer-3-sigma-update, ccl-v3-2-1-correlation-has-4pi, cosmocov-pure-e-b-covariance, decision-exclude-snakemake-rule, decision-keep-v1-4-6-in-cat) are genuinely disconnected from the main flow and all show "resolved" via their own decisions/insights. **Gotcha:** `felt export --format astra` alphabetizes YAML keys — primary insight key must sort before secondary ones for correct verdict display in graph.ts.
