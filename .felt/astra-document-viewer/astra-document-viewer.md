...
### What's been built (as of 2026-04-02, updated 2026-04-06)
...
- **Topological DAG layout**: X = data-flow depth (longest path from source), Y = tag-affinity sorted within column. Periphery (unconnected nodes) in rightmost column. Root anchored left. Replaced earlier two-column layout. **Layout fix (2026-04-06)**: DAG structure now computed first; `nodeRy` and `nodeRx` sized to fit both vertical (max nodes per column) and horizontal (column count) constraints — prevents overlap when graph has 7+ columns.
...
- **Data-flow cross-references**: 36 links (updated 2026-04-06, was 31). Extended chain: `350-mock→{covariance,96-bin}`, `cosebis-angular-binning→data-vectors`, `96-bin→data-vectors`, `switch-fiducial→data-vectors`, `{covariance,data-vectors}→scale-cuts`, `{scale-cuts,blind-b-is-physical}→paper-synthesis`. Validation analyses (cosebis-angular-binning, 96-bin-powspace) now connected to spine rather than isolated in periphery. Click navigation confirmed working.
...
- **Keep enriching the analysis.** 21 nodes formalized. Spine complete. Peripheral validation analyses now connected: `cosebis-angular-binning` enriched with insights/decisions (now "resolved" with verdict), `96-bin-powspace-is-optimal` data-flow fixed. 18/21 nodes now show "resolved" in map. Remaining periphery (6 nodes: blinding-framing-for-talk, bump-no-longer-3-sigma-update, ccl-v3-2-1-correlation-has-4pi, cosmocov-pure-e-b-covariance, decision-exclude-snakemake-rule, decision-keep-v1-4-6-in-cat) are genuinely disconnected from the main flow. Consider enriching `350-mock-glass-covariance-cl` and `bb-covariance-blind` (currently "open") with insights. **Gotcha:** `felt export --format astra` alphabetizes YAML keys — primary insight key must sort before secondary ones for correct verdict display in graph.ts.
