...
- **Keep enriching the analysis.** 21 nodes now formalized. All 5 tier:1 spine nodes are complete: `covariance → data-vectors → {scale-cuts, version-comparisons} → paper-synthesis` with `blind-b-is-physical` feeding into `paper-synthesis`. The scientific argument is readable left-to-right via verdict sentences. Consider formalizing 2-3 peripheral decision nodes (CCL offset, COSEBIS binning) to enrich depth, but the analysis spine is done. **Gotcha:** `felt export --format astra` alphabetizes YAML keys — primary insight key must sort before secondary ones for correct verdict display in graph.ts.
